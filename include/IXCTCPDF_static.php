...
class IXCTCPDF_STATIC
{
    ...
    {
        return tempnam(K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(IXCTCPDF_STATIC::getRandomSeed()).'_');
    }
    ...
    * @since 5.9.121 (2011-09-28)
    ...
    public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl = false)
    {
        ...
        if (isset($prop['fillColor'])) {
            if (is_array($prop['fillColor'])) {
                ...
                $opt['mk']['bg'] = IXCTCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
            }
        }
        ...
        if (isset($prop['strokeColor'])) {
            if (is_array($prop['strokeColor'])) {
                ...
                $opt['mk']['bc'] = IXCTCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
            }
        }
        ...
        foreach ($cssblocks as $key => $block) {
            if (strpos($block[0], ',') > 0) {
                ...
                foreach ($selectors as $sel) {
                    $cssblocks[] = [0 => trim($sel), 1 => $block[1]];
                }
                unset($cssblocks[$key]);
            }
        }
    ...
    public static function getHyphenPatternsFromTEX($file)
    {
        ...
        foreach ($patterns_array as $val) {
            if (! IXCTCPDF_STATIC::empty_string($val)) {
                $val = trim($val);
                $val = str_replace('\'', '\\\'', $val);
