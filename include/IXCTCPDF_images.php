...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    public static function _parsejpeg($file)
    {
        ...
        if (! @IXCTCPDF_STATIC::file_exists($file)) {
            return false;
        }
        $a = getimagesize($file);
        if (empty($a)) {
        ...
        while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
            ...
            $length = (IXCTCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
    ...
    public static function _parsepng($file)
    {
        ...
        $w = IXCTCPDF_STATIC::_freadint($f);
        $h = IXCTCPDF_STATIC::_freadint($f);
        $bpc = ord(fread($f, 1));
        $ct = ord(fread($f, 1));
        if ($ct == 0) {
            $colspace = 'DeviceGray';
        ...
        $data = '';
        $icc = false;
        $n = IXCTCPDF_STATIC::_freadint($f);
        do {
            $type = fread($f, 4);
            if ($type == 'PLTE') {
                // read palette
                $pal = IXCTCPDF_STATIC::rfread($f, $n);
                fread($f, 4);
            } elseif ($type == 'tRNS') {
                // read transparency info
                $t = IXCTCPDF_STATIC::rfread($f, $n);
                if ($ct == 0) { // DeviceGray
                    $trns = [ord($t[1])];
                ...
                $data .= IXCTCPDF_STATIC::rfread($f, $n);
                fread($f, 4);
            } elseif ($type == 'iCCP') {
                // skip profile name
                $len = 0;
                ...
                $icc = IXCTCPDF_STATIC::rfread($f, ($n - $len - 2));
                // decompress profile
