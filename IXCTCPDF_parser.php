...
require_once(dirname(__FILE__).'/include/IXCTCPDF_filters.php');
...
// -----------------------------------------------------------------------------
    ...
    protected function decodeStream($sdic, $stream)
    {
        ...
        foreach ($filters as $filter) {
            if (in_array($filter, IXCTCPDF_FILTERS::getAvailableFilters())) {
                try {
                    $stream = IXCTCPDF_FILTERS::decodeFilter($filter, $stream);
                } catch (Exception $e) {
                    $emsg = $e->getMessage();
